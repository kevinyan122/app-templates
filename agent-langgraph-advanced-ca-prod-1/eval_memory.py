"""Run categorized, multi-turn evaluations for the managed-memory agent.
...
Examples:
    ...
    uv run python eval_memory.py --scenario explicit-save-coffee-preference
    uv run python eval_memory.py --category implicit-search
    uv run python eval_memory.py --dataset
    uv run python eval_memory.py --no-judges
"""
...
from memory_eval_cases import CATEGORIES, SCENARIOS, validate_scenarios  # noqa: E402
...
def build_records(scenarios: list[dict[str, Any]] = SCENARIOS) -> list[dict[str, Any]]:
...
def filter_records(
    ...
    if unknown := categories - CATEGORIES:
...
def snapshot_scope(scope: str) -> dict[str, dict[str, str]]:
    ...
    for listed in response.get("entries", []):
        path = listed["path"]
        ...
        snapshot[path] = _clean_entry(entry)
    return snapshot
...
def _invoke_function():
    ...
    from agent_server import agent as _agent  # noqa: F401 - registers the handler

    invoke_fn = get_invoke_function()
    if invoke_fn is None:
        raise RuntimeError("The agent invoke handler was not registered.")
    return invoke_fn


async def _invoke_turn(message: str, scope: str, thread_id: str) -> dict[str, Any]:
    from mlflow.types.responses import ResponsesAgentRequest

    request = ResponsesAgentRequest(
        input=[{"role": "user", "content": message}],
        custom_inputs={"user_id": scope, "thread_id": thread_id},
    )
    response = await _invoke_function()(request)
...
def run_scenario(
    ...
    scope = f"{SCOPE_PREFIX}-{RUN_TOKEN}-{invocation_token}-{safe_id}"
    session_number = 1
    ...
    try:
        ...
        for turn_index, turn in enumerate(turns):
            if turn_index and turn.get("new_session", False):
                session_number += 1
            thread_id = f"{scope}-session-{session_number}"
            with _turn_trace(turn_index, turn, session_number) as turn_span:
                before = confirmed_snapshot
                response = _run_async(_invoke_turn(turn["message"], scope, thread_id))
...
def _run_label(records: list[dict[str, Any]]) -> str:
    ...
    if len(scenario_ids) == len(SCENARIOS):
        return "full-suite"
    return f"{len(scenario_ids)}-scenario-smoke"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", action="store_true", help="Load cases from the MLflow dataset")
    ...
    parser.add_argument("--no-judges", action="store_true", help="Run deterministic scorers only")
    parser.add_argument("--index-timeout", type=int, default=INDEX_TIMEOUT_S)
    return parser.parse_args()


def main() -> None:
    global INDEX_TIMEOUT_S

    args = parse_args()
    INDEX_TIMEOUT_S = args.index_timeout
    # MLflow evaluates rows concurrently by default. Keep the first version reproducible;
    # scenario-level concurrency can be enabled later if runtime becomes a problem.
    os.environ.setdefault("MLFLOW_GENAI_EVAL_MAX_WORKERS", "1")
    # The default preflight calls predict_fn once before the recorded evaluation. A stateful
    # memory scenario must execute exactly once or its just-deleted search result can pollute
    # the real run while the search index catches up.
    os.environ.setdefault("MLFLOW_GENAI_EVAL_SKIP_TRACE_VALIDATION", "true")

    records = (
        load_records_from_dataset(args.dataset_name)
        if args.dataset
        else build_records()
    )
    records = filter_records(records, set(args.scenario), set(args.category))
    ...
    source = args.dataset_name if args.dataset else "memory_eval_cases.py"
    print(f"Running {len(records)} scenarios from {source}")
    print("Judges:", "off" if args.no_judges else "memory_write_quality, memory_answer_quality")

    scenario_ids = [str(record["inputs"]["scenario_id"]) for record in records]
    categories = sorted({str(record["inputs"]["category"]) for record in records})
    run_name = f"memory-eval/v2/{_run_label(records)}"
    run_tags = {
        "suite": "managed-memory-v2",
        "case_source": source,
        "git_sha": _git_sha(),
        "judges_enabled": str(not args.no_judges).lower(),
        "judge_names": (
            "memory_write_quality,memory_answer_quality" if not args.no_judges else "none"
        ),
        "scenario_count": str(len(records)),
        "scenario_ids": ",".join(scenario_ids),
        "categories": ",".join(categories),
        "index_timeout_seconds": str(args.index_timeout),
    }

    mlflow.set_experiment(experiment_id=EXPERIMENT_ID)
    with mlflow.start_run(run_name=run_name, tags=run_tags):
        result = mlflow.genai.evaluate(data=records, predict_fn=predict_fn, scorers=scorers)
    client = mlflow.MlflowClient()

    log_result_breakdown(result, client)
