...
- **Short-term memory** uses the beta Unity Catalog Conversations API when `DATABRICKS_CONVERSATIONS_ENABLED=true`, with `AsyncCheckpointSaver` as the Lakebase-backed fallback.
...
2. Reuse `custom_outputs.managed_conversation.id` as `context.conversation_id`; when Conversations API is disabled, reuse `thread_id` instead (see **agent-memory** skill)
...
> **Which to use.** Both persist facts across sessions and support semantic retrieval. Stick with the default **Lakebase** store when you need direct SQL access, control over the embedding/index infrastructure, or it's already part of your stack. Choose **managed memory** for service-managed semantic retrieval with BM25 keyword boosting and UC governance, without memory infrastructure to run.
