...
import {
  ...
  type ManagedConversationData,
  managedConversationDataSchema,
  StreamCache,
  type VisibilityType,
...
} from '../lib/managed-conversations';

export const chatRouter: RouterType = Router();

const streamCache = new StreamCache();

function findManagedConversation(
  messages: ChatMessage[],
): ManagedConversationData | null {
  for (
    ...
    for (let partIndex = parts.length - 1; partIndex >= 0; partIndex--) {
      ...
      if (part?.type !== 'data-managedConversation') continue;
...
chatRouter.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let managedConversation = findManagedConversation(uiMessages);
    const streamId = generateUUID();

    const model = await myProvider.languageModel(selectedChatModel);
    const modelMessages = await convertToModelMessages(uiMessages);
    const requestHeaders = {
      [CONTEXT_HEADER_CONVERSATION_ID]: managedConversation?.id ?? id,
    ...
    const result = streamText({
      ...
      onChunk: ({ chunk }) => {
        if (chunk.type === 'raw') {
          ...
          if (raw?.type === 'response.conversation.ready') {
            const parsed = managedConversationDataSchema.safeParse(
              raw?.custom_outputs?.managed_conversation,
            );
            if (parsed.success) {
              managedConversation = parsed.data;
            } else {
              console.warn(
                'Ignoring invalid managed conversation stream payload',
                parsed.error,
              );
            }
          }
...
chatRouter.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    await deleteManagedConversations({
      conversationIds: findManagedConversationIds(messages),
      forwardedAccessToken: req.headers['x-forwarded-access-token'] as
        | string
        | undefined,
    });
