...
test.describe
  .serial('Context Injection', () => {
    ...
    test.describe('agent/v1/responses endpoints', () => {
      ...
      test('reuses the managed conversation id on later turns', async ({
        adaContext,
      }) => {
        const chatId = generateUUID();
        const conversationId = 'conv_managed_session_123';
        const response = await adaContext.request.post('/api/chat', {
          data: {
            id: chatId,
            message: {
              id: generateUUID(),
              role: 'user',
              parts: [{ type: 'text', text: 'Continue this conversation.' }],
            },
            previousMessages: [
              {
                id: generateUUID(),
        ...
        const capturedRequests =
          (await capturedResponse.json()) as CapturedRequest[];
        const chatRequest = capturedRequests.find(
          (request) => request.context?.conversation_id === conversationId,
        );
