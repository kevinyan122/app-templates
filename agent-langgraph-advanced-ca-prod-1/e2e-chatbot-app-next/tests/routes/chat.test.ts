...
test.describe
  .serial('/api/chat', () => {
    ...
    test('managed conversation id is preserved as a typed UI data part', async ({
      adaContext,
    }) => {
      const chatId = generateUUID();
      const response = await adaContext.request.post('/api/chat', {
        data: {
          ...
          message: {
            ...
            role: 'user',
            parts: [
              { type: 'text', text: 'Trigger managed conversation display' },
            ],
          },
          selectedChatModel: 'chat-model',
          selectedVisibilityType: 'private',
      ...
      const conversationPart = payloads.find(
        (payload) => payload.type === 'data-managedConversation',
      );

      expect(conversationPart?.data).toEqual({
        id: 'conv_mock_session_123',
        created: true,
      });
    });
