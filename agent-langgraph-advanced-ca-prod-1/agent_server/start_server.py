...
_lb = LAKEBASE_CONFIG  # None when no Lakebase is configured (background mode then disabled)
agent_server = AgentServer(
    "ResponsesAgent",
    enable_chat_proxy=True,
    db_instance_name=_lb.instance_name if _lb else None,
    db_autoscaling_endpoint=_lb.autoscaling_endpoint if _lb else None,
    db_project=_lb.autoscaling_project if _lb else None,
    db_branch=_lb.autoscaling_branch if _lb else None,
...
async def _lifespan(app):
    if LAKEBASE_CONFIG is None:
        # No Lakebase — skip checkpointer setup; long-term managed memory still works.
        logger.info("No Lakebase configured — starting without a short-term checkpointer.")
        async with _original_lifespan(app):
            yield
        return
    try:
        async with lakebase_context(LAKEBASE_CONFIG) as checkpointer:
            await checkpointer.setup()
            logger.info("Lakebase setup complete")

            app.state.checkpointer = checkpointer
            set_lakebase_resources(checkpointer)
