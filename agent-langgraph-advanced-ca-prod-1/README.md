...
Short-term memory can use either of two backends:

- With `DATABRICKS_CONVERSATIONS_ENABLED=true`, the beta Unity Catalog Conversations API stores OpenAI-compatible messages and tool items in the configured `DATABRICKS_MEMORY_STORE`. The first response returns `custom_outputs.managed_conversation.id`; pass that value as `context.conversation_id` on later API calls. The bundled chat UI does this automatically.
- Otherwise, a configured Lakebase instance enables the LangGraph `AsyncCheckpointSaver`; pass a stable `thread_id` in `custom_inputs`.

Managed conversations and the LangGraph checkpointer are alternative owners of the same transcript; the app never enables both for one request.
The deployed app must request the `unity-catalog` user API scope so its forwarded OBO token can call the Conversations API.

### Long-term memory (user facts)

Long-term memory persists user preferences and information across sessions in the configured Unity Catalog managed memory store. The agent searches memory on every user turn and can search, read, list, save, update, or delete entries. Deployed requests derive the memory scope from the verified forwarded user identity rather than a model-selected scope.
...
With managed conversations enabled, retain the conversation ID from the first response and send it with the new input on later turns:

```bash
curl -X POST <base_url>/responses \
  -H "Content-Type: application/json" \
  -d '{
    "input": [{ "role": "user", "content": "What did I say earlier?" }],
    "context": { "conversation_id": "conv_..." },
...
| Variable | Description | Default |
|----------|-------------|---------|
| `DATABRICKS_MEMORY_STORE` | Unity Catalog managed memory store used for long-term memory and managed conversations | _(required)_ |
| `DATABRICKS_CONVERSATIONS_ENABLED` | Enable beta Conversations API short-term history | `false` |

**Lakebase (optional checkpointer and required for background mode):**

Lakebase can provide the fallback LangGraph checkpointer and background-mode persistence. Configure either a provisioned instance OR an autoscaling project/branch — not both.
...
If no Lakebase variables are set, background mode and the fallback checkpointer are disabled. Managed long-term memory and Conversations API can still operate through `DATABRICKS_MEMORY_STORE`.
